...
module.exports = async (Bumper, member) => {
  ...
  if (!channel)
    return (
      Quick.delete(`welcomeLog_${member.guild.id}`),
      console.log("Channel not found")
    );

  let msg;
  let message = await Quick.fetch(`welcomeMSG_${member.guild.id}`);
  if (message === null) {
    msg = `Welcome To The Server ${member.user}`;
  }
  if (message) {
    msg = `Welcome! ${member.user} ${message}`;
  }
  ...
  channel.setTopic(
    `${member.guild.name} Now has #${member.guild.members.size} members`
  );
};
